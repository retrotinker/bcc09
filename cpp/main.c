...
char **argv;
{
	...
	char *p;
	static char Usage[] =
	    "Usage: cpp -E -0 -Dxxx -Uxxx -Ixxx infile -o outfile";
	...
	for (ar = 1; ar < argc; ar++)
		if (argv[ar][0] == '-')
			...
			case '3':
				...
				break;
			case 'c':
				define_macro("__CALLER_SAVES__");
				break;
			case 'f':
				define_macro("__FIRST_ARG_IN_AX__");
				break;
			case 'O':
				define_macro("__OPTIMISED__");
				break;
			...
			case 'I':
				...
				if (i >= MAXINCPATH)
					cfatal
					    ("Too many items in include path for CPP");
				break;
			case 'D':
				if (argv[ar][2])
					p = argv[ar] + 2;
	...
	if (dialect != DI_KNR) {
		...
		sprintf(buf, "__DATE__=\"%.3s %.2s %.4s\"", timep + 4,
			timep + 8, timep + 20);
		define_macro(buf);
	}
...
FILE *open_include(fname, mode, checkrel)
char *fname;
char *mode;
int checkrel;
{
...
void print_toks_cpp()
{
	...
	while ((i = gettok()) != EOF) {
		...
		case TK_FILE:
			sprintf(curword, "\"%s\"", c_fname);
			if (0) {
		case TK_LINE:
				sprintf(curword, "%d", c_lineno);
			}
		/*FALLTHROUGH*/ default:
			if (!alltok) {
...
void print_toks_raw()
{
	...
	while ((i = gettok()) != EOF) {
		...
		case '\'':
			if (debug_mode < 2) {
				fprintf(ofd, "%-16s: %s", "Quoted string",
					curword);
				while ((i = gettok()) == TK_STR)
					outpos += fprintf(ofd, "%s", curword);
				if (i == '\n')
