		...
		if (!quoted_str) {
			...
			if (ch == TK_WORD) {
				struct token_trans *p =
				    is_ckey(curword, strlen(curword));
				if (p)
					return p->token;
			}
...
break_break:
	...
	if (state == 1) {
		...
		unchget(ch);
		if (!dont_subst
		    && (ptr = read_entry(0, curword)) != 0 && !ptr->in_use) {
			if (def_count >= MAX_DEFINE) {
				...
				gen_substrings(ptr->name, ptr->value,
					       ptr->arg_count, ptr->varargs);
...
static int do_proc_copy_hashline()
{
	int off, ch;
	...
	if ((ch = gettok_nosub()) == TK_WORD) {
		...
		while (ch != '\n') {
			if (cc + 4 > len) {
				len = cc + WORDSIZE;
				ptr =
				    (struct define_item *)realloc(ptr,
								  sizeof(struct
									 define_item)
								  + len);
				if (ptr == 0)
...
static void do_proc_tail()
{
	...
	while ((ch = pgetc()) != '\n')
		if (ch > ' ') {
			if (!if_false && flg)
				cwarn
				    ("Unexpected text following preprocessor command");
			flg = 0;
		}
}

static int get_if_expression()
{
...
static int_type get_expression(prio)
int prio;
{
	int_type lvalue;
	int_type rvalue;
		...
		if (strcmp("defined", curword) == 0) {
			...
			if (curtok == '(' && gettok_nosub() != TK_WORD)
				...
				if (curtok == '(' && gettok_nosub() != ')')
					cerror
					    ("'defined' keyword requires closing ')'");
				else
					curtok = get_onetok(SKIP_SPACE);
			}
		} else
...
void gen_substrings(macname, data_str, arg_count, is_vararg)
char *macname;
char *data_str;
int arg_count;
int is_vararg;
...
static char *insert_substrings(data_str, arg_list, arg_count)
char *data_str;
struct arg_store *arg_list;
int arg_count;
{
