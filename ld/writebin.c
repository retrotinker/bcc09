...
bool_pt arguzp;
{
...
#endif
	{
		...
		if (seg == 3 && uzp && !stripflag) {	/* XXX Stripped last seek needed */
			bin_off_t val;
			val =
			    ld_roundup(segbase[seg] + segsz[seg], page_size(),
				       bin_off_t);
	...
	*(unsigned short *)&header.a_magic = OMAGIC;	/* XXX - works for 386BSD */
...
#ifdef QMAGIC
	if (uzp)
		offtocn((char *)&header.a_text,
			etextpadoff - btextoffset + HEADERLEN,
...
#endif
	{
		...
		offtocn((char *)&header.a_total, (bin_off_t)
			(endoffset <
			 0x00010000L ? 0x00010000L : endoffset + 0x0008000L),
