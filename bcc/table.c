...
struct typestruct *type;
{
	...
	symptr = locptr;
	locptr =
	    (struct symstruct *)align(&symptr->name.namea[strlen(name) + 1]);
...
PUBLIC void dumpglbs()
{
	...
	for (i = 0; i < HASHTABSIZE; ++i)
		for (symptr = hashtab[i]; symptr != NULL; symptr = symptr->next)
			if (symptr->storage == GLOBAL
			    && !(symptr->flags & EXTERNAL)
			    && *symptr->name.namea >= 'A'
	...
	for (i = 0; i < HASHTABSIZE; ++i)
		for (symptr = hashtab[i]; symptr != NULL; symptr = symptr->next)
			if (symptr->storage == LOCAL)
				set(symptr->name.namea,
				    symptr->offset.offi - sp);
}
...
PUBLIC void newlevel()
{
	...
	if (level >= MAXLEVEL)
		limiterror
		    ("compound statements nested too deeply (126 levels)");
	else
		++level;
}
...
PUBLIC void syminit()
{
	...
	for (tdptr = scaltypes; tdptr < scaltypes + NSCALTYPES; ++tdptr) {
		(*tdptr->tdtypeptr = type = newtype())->scalar =
		    tdptr->tdscalar;
		type->alignmask =
		    ~((type->typesize = tdptr->tdsize) - 1) | alignmask;
