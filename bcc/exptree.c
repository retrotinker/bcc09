	...
	if ((bothscalar = lscalar | rscalar) & RSCALAR) {
		...
		if (!(lscalar & DOUBLE))
			nodeptr->left.nodeptr =
			    castnode(dtype, nodeptr->left.nodeptr);
		if (!(rscalar & DOUBLE))
			nodeptr->right = castnode(dtype, right);
	} else if (!(bothscalar & DLONG) && ((nodeptr->tag == ANDOP &&
					      (redscalar(nodeptr->left.nodeptr)
					       | redscalar(right)) & CHAR)
					     ||
					     ((nodeptr->tag == EOROP
					       || nodeptr->tag == OROP)
					      && redscalar(nodeptr->left.
							   nodeptr) &
					      redscalar(right) & CHAR)
					     || (nodeptr->tag == MODOP
						 && right->tag == LEAF
						 ...
						 storage == CONSTANT
						 && (divisor =
						     right->left.symptr->offset.
						     offv,
						     (uvalue_t) divisor <=
						     MAXUCHTO + 1)
						 && bitcount((uvalue_t) divisor)
						 <= 1))) {
		...
		if (bothscalar & (DLONG | SHORT | INT)) {
			...
			if (!(lscalar & LONG))
				nodeptr->left.nodeptr =
				    castnode(nodeptr->nodetype,
					     nodeptr->left.nodeptr);
			if (!(rscalar & LONG))
				nodeptr->right =
				    castnode(nodeptr->nodetype, right);
		}
...
register struct nodestruct *nodeptr;
{
	...
	return nodeptr->tag == LEAF &&
	    ...
	    symptr->offset.offv == 0 && symptr->type->scalar & ISCALAR;
}
...
struct nodestruct *p2;
{
	...
	case CONDOP:
		...
		if (p2->nodetype->constructor & STRUCTU) {
			p2 = node(COLONOP,
				  node(ADDRESSOP, p2->left.nodeptr, NULLNODE),
	...
	case SUBOP:
		...
		if (p2->tag == LEAF && p2->nodetype->scalar
			...
			return node((op_t) t == SUBOP ? ADDOP : ADDABOP, p1,
				    p2);
		}
#endif
		if (p1->nodetype->constructor & (ARRAY | POINTER)) {
	...
	case CASTOP:
		...
		if (((lscalar = p1->nodetype->scalar) | rscalar) & RSCALAR) {
			...
			if (lscalar & RSCALAR && !(rscalar & RSCALAR)) {
				...
				static double MAXULONG =
				    (double)0xFFFFFFFFL + 1;
			...
			if (!(lscalar & RSCALAR) && rscalar & RSCALAR) {
				...
				target->offset.offd =
				    qmalloc(sizeof *target->offset.offd);
				if (lscalar & UNSIGNED)
		...
		if (target->storage == CONSTANT) {
			if (rscalar & CHAR) {
				...
				if (!(rscalar & UNSIGNED)
				    && target->offset.offv > maxshortto)
					target->offset.offv -=
					    (maxushortto + 1);
			} else if (rscalar & INT) {
				target->offset.offv &= intmaskto;
				if (!(rscalar & UNSIGNED)
				    && target->offset.offv > maxintto)
					target->offset.offv -= (maxuintto + 1);
			} else if (rscalar & FLOAT)
				*target->offset.offd =
				    (float)*target->offset.offd;
		} else
		    if ((targszdelta =
			 ((p1->nodetype->
			   constructor & (ARRAY | POINTER)) ? ptypesize : p1->
			  nodetype->typesize) - p2->nodetype->typesize) == 0) ;
...
struct nodestruct *nodeptr;
{
	...
	case COLONOP:
		if (isnodecharconst(left) && isnodecharconst(right)) {
			targtype =
			    (targtype->scalar | right->nodetype->
			     scalar) & UNSIGNED ? uctype : ctype;
		...
		if ((lscalar = targtype->scalar) != 0 &&
			...
			if ((!(bothscalar & INT)
				...
				if (left->nodetype != targtype)
					nodeptr->left.nodeptr =
					    castnode(targtype, left);
				if (right->nodetype != targtype)
					nodeptr->right =
					    castnode(targtype, right);
			}
			return targtype;	/* XXX - reduced from promote(targtype) */
		}
	...
	case ASSIGNOP:
		...
		if ((targtype->constructor | right->nodetype->constructor)
			...
			error
			    ("assignment to/from struct/union of a different type");
			fixnode(nodeptr);
			return errtype;
		}
		if (!(left->flags & LVALUE)) {
...
struct nodestruct *nodeptr;
{
	if (nodeptr->nodetype->constructor & (ARRAY | FUNCTION))
		return
		    castnode(pointype
			     (nodeptr->nodetype->constructor & ARRAY ? nodeptr->
			      nodetype->nexttype : nodeptr->nodetype), nodeptr);
	return nodeptr;
}
