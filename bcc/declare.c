...
struct typestruct *sufftype;
{
	if (pretype->nexttype != NULL) {
		...
		if (pretype->constructor != ARRAY)
			return prefix(pretype->constructor, pretype->typesize,
				      sufftype);
		else
			return prefix(ARRAY,
				      pretype->typesize /
				      pretype->nexttype->typesize *
...
PRIVATE void declarg()
{
	...
	else if (gvarsymptr->flags & INITIALIZED
	...
	while (sym != SEMICOLON && sym != EOFSYM) {
		...
		if (gvarname[0] == 0) {
			...
			if (findlorg(gvar2name) != NULL)
				...
				(gvarsymptr =
				 addlorg(gvar2name, gvartype))->storage =
		       NOSTORAGE;
...
PRIVATE void declfunc()
{
	...
	for (symptr = &locsyms[0];
	     ...
	     *symptr->name.namea >= 'A'; symptr = (struct symstruct *)
...
PRIVATE void idecllist()
{
	...
	while (sym != SEMICOLON) {
		...
		if (level == ARGLEVEL) {
...
#ifdef DIRECTPAGE
			/*
			 * make all global scalar and pointer variables
			 * DIRECTPAGE only iff generating position
			 * independent code
			 */
			if (posindependent &&
			    !(gvartype->constructor & ~POINTER))
			...
			if ((gvarsc != REGDECL || !regdecl()) &&
			    gvartype->constructor != FUNCTION
			    && gvarsc != STATICDECL && gvarsc != TYPEDEFDECL)
				gvarsymptr->offset.offi = softsp =
				    (softsp & gvartype->alignmask) -
				    gvartype->typesize;
		}
		if (gvarsc == STATICDECL) {
			gvarsymptr->flags &= ~EXTERNAL;
		...
		if (sym == ASSIGNOP) {
			...
			if (level == GLBLEVEL || gvarsc == STATICDECL) {
				...
				if (gvarsc == EXTERNDECL)
					error
					    ("initialization of externs is illegal");
				switch (gvartype->constructor) {
				case ARRAY:
					error
					    ("initialization of auto arrays is illegal");
					break;
				case FUNCTION:
					error
					    ("initialization of functions is illegal");
					break;
				default:
...
struct typestruct *type;
{
	...
	if (dimension == 0) {
		if (type == gvartype)
			gvartype =
			    prefix(ARRAY, -remaining * basesize, basetype);
	} else
...
struct typestruct *type;
{
	...
	if ((typelist = type->listtype) != NULL)
		do {
			...
			newoffset =
			    (offset + ~memtype->alignmask) & memtype->alignmask;
