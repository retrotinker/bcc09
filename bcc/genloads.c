...
store_pt targreg;
{
	if (source->type->scalar & DLONG) {
		...
		if (source->storage == CONSTANT) {
			{
				int regs, i, off = 1;
				loadconst(((unsigned short *)source->offset.
					   offd)[0], DREG);
				regs = (targreg & ~DREG);
				for (i = 1; i; i <<= 1) {
					if (regs & i)
						loadconst(((unsigned short *)
							   source->offset.
							   offd)[off++], i);
				}
			}
		...
		if (source->indcount == 0) {
			...
			if (source->offset.offi != 0)
				bugerror
				    ("loading direct float with offset not implemented");
		} else
			loadlongindirect(source, targreg);
...
store_pt targreg;
{
	...
	if (source->storage == CONSTANT) {
		if (source->type->scalar & CHAR
		    && (store_t) targreg & ALLDATREGS)
			targreg = BREG;
		longlow = (offset_T) source->offset.offv;
		if (source->type->scalar & DLONG) {
			longlow &= (offset_T) intmaskto;
			longhigh =
			    (offset_T) (source->offset.offv >> INT16BITSTO)
			    & (offset_T) intmaskto;
...
struct symstruct *adr;
{
	...
	case INDREG2:
		...
		if (indflag && adr->offset.offi != 0
		    && is5bitoffset(adr->offset.offi))
			bumplc();
...
struct symstruct *target;
{
	...
	if (target->indcount == 0) {
		if (target->offset.offi != 0 || target->level == OFFKLUDGELEVEL
		    || !(target->storage & allregs)
		    || target->storage & CHARREGS)
			bugerror("bad register store");
		else if ((store_t) (targreg = target->storage) !=
			 (store_t) sourcereg) {
