...
#ifdef BUILTIN_CPP
		if (maclevel == 0)
			outudec((unsigned)(lineptr - inputbuf->fbuf) -
				input.lineoffset);
		else {
	...
	*charptr++ = 0;
	...
	do {
		if (incptr->incdirname == NULL) {
			...
			fullnameptr =
			    ourmalloc(dirnamelen + (int)(charptr - fnameptr)
				      + 2);
...
char *argv[];
{
	...
	for (argn = 1; argn < argc; ++argn) {
		...
		if (*arg != '-') {
			...
			case 'I':
				(incnew =
				 (struct inclist *)ourmalloc(sizeof *incnew))
				    ->incdirname = arg + 2;
#ifdef TS
				++ts_n_includelist;
