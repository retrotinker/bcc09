...
int count;
{
	...
	while (gap != 1) {
		...
		for (j = gap; j < count; ++j)
			for (i = j - gap;
			     i >= 0
			     && caselist[i].casevalue >
			     caselist[i + gap].casevalue; i -= gap) {
				swaptemp = caselist[i];
		...
		if (switchnow->charselector && (caseval < 0 || caseval > 255))
			...
			if (switchnow->caseptr >= switchnow->casetop) {
				...
				ncases = ( /* ptrdiff_t */ int)
				    (switchnow->caseptr -
				     &switchnow->caselist[0]);
				switchnow =
				    realloc(switchnow, ( /* size_t */ unsigned)
					    ((char *)switchnow->caseptr -
					     (char *)switchnow)
					    +
					    GROWCASES *
					    sizeof(struct casestruct));
#ifdef TS
				++ts_n_case_realloc;
				ts_s_case +=
				...
				if (switchnow == NULL)
					outofmemoryerror("");
				switchnow->caseptr =
				    &switchnow->caselist[ncases];
				switchnow->casetop =
				    switchnow->caseptr + GROWCASES;
			}
...
PRIVATE void jumptocases()
{
	...
	while (caseptr <= casetop) {
		...
		for (case1ptr = caseptr; case1ptr < casetop; ++case1ptr)
			if (case1ptr->casevalue <
			    (case1ptr + 1)->casevalue - 10)
				break;
		if (case1ptr < caseptr + 5) {
			...
			outcmp();
			outimadj((offset_T) (case1ptr->casevalue - basevalue),
				 targreg);
			...
			for (caseval = caseptr->casevalue;
			     caseval <= case1ptr->casevalue; ++caseval) {
				defword();
